...
| `void.ts` | An app's `void.ts`: the customer state of record with an AI Gateway model declared as a meter via `@void/sdk/ai`'s `metered` — per-token-class pricing, quota entitlements, and a margin invariant fed by automatic `_cost`. |
